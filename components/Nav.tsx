...
export default function Nav({ isAdmin, customerName }: { isAdmin: boolean; customerName: string | null }) {
  return <>
    ...
    <div className="offcanvas offcanvas-end mobile-drawer" tabIndex={-1} id="mobileNav" aria-labelledby="mobileNavLabel">
      ...
      <div className="offcanvas-body"><ul className="navbar-nav mobile-nav-list"><NavLinks /></ul><CartLink /><AccountControl isAdmin={isAdmin} customerName={customerName} /><button className="theme-toggle mobile-theme-toggle" type="button" aria-label="Toggle light and dark mode"><i className="bi bi-moon-stars" /><span>Theme</span></button></div>
    </div>
  </>;
}
