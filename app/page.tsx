...
const HOME_TITLE = "LONG SENGCHHUN | Multidisciplinary Creative Designer";
const HOME_DESCRIPTION = "Portfolio and creative services by LONG SENGCHHUN, a multidisciplinary creative designer based in Phnom Penh, Cambodia.";
...
export default async function HomePage() {
  const [{ site, social }, featured, videoProject, services, software] = await Promise.all([getSiteContext(), getFeaturedProjects(), getFeaturedVideoProject(), getServices(), getSoftwareTools()]);
  const jsonLd = {
  ...
  return <>
    ...
    <section className="section"><div className="container"><div className="section-heading"><div><p className="eyebrow">Featured Work</p><h2>Selected projects from the portfolio.</h2></div></div><div className="project-grid editorial-grid">{featured.length ? featured.map((project) => <ProjectCard project={project} key={project.id} />) : <div className="empty-state"><p>No featured projects yet.</p></div>}</div></div></section>
    ...
    <section className="section"><div className="container split-section"><div className="reveal"><p className="eyebrow">About</p><h2>Creative work across design, motion, film, and 3D.</h2></div><div className="prose reveal"><p>I am a multidisciplinary creative based in Cambodia, working across graphic design, photography, filmmaking, video editing, and 3D visualization.</p><p>Based in Phnom Penh, Cambodia. Available for freelance collaborations and for working with local and international clients when opportunities are available.</p><Link className="text-link" href="/about/">Learn more</Link></div></div></section>
