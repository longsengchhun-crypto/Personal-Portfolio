...
export default async function HomePage() {
  const [{ site, social }, videoProject, services, software] = await Promise.all([getSiteContext(), getFeaturedVideoProject(), getServices(), getSoftwareTools()]);
  const jsonLd = {
  ...
  return <>
    ...
    <HeroSlider slides={HERO_SLIDES}><p className="eyebrow">{OWNER.name}</p><h1>{site?.professional_title || OWNER.title}</h1><p className="role-line">{OWNER.roles}</p><p className="hero-description">{site?.hero_intro || "Creating visual experiences through production, post-production, and digital creativity."}</p><p className="khmer-line">{site?.khmer_intro || KHMER.hero}</p><div className="button-row"><Link className="btn btn-accent" href="/showreel/"><i className="bi bi-play-fill" />Watch Showreel</Link><Link className="btn btn-outline-light" href="/portfolio/">View Selected Work</Link></div></HeroSlider>
