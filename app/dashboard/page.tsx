...
export default async function DashboardPage({ searchParams }: { searchParams: Promise<{ email?: string; sms?: string; action?: string }> }) {
  ...
  return <section className="dashboard-console"><div className="container">
    <header className="console-head">
      ...
      <div className="console-actions"><a className="btn btn-outline-light" href="https://supabase.com/dashboard/project/dyjzccnatslknumnbplj/editor" target="_blank" rel="noreferrer"><i className="bi bi-sliders" />Supabase</a><form method="post" action="/api/dashboard/logout/"><button className="btn btn-outline-light" type="submit"><i className="bi bi-box-arrow-right" />Log Out</button></form></div>
    </header>
    ...
    <div className="dashboard-operations"><DashboardLiveRefresh /><div className="delivery-readiness" aria-label="Client notification readiness"><span className={emailReady ? "is-ready" : "needs-setup"} title={emailReadiness.message}><i className={`bi ${emailReady ? "bi-envelope-check" : "bi-envelope-exclamation"}`} />Client email: {emailReady ? "Ready" : emailReadiness.mode === "testing" ? "Test mode" : "Sender invalid"}</span><span className={smsReady ? "is-ready" : "needs-setup"}><i className={`bi ${smsReady ? "bi-chat-square-check" : "bi-chat-dots"}`} />Optional SMS: {smsReady ? "Ready" : "Off"}</span></div></div>
    ...
    <div className="console-layout analytics-layout">
      <section className="console-panel visitor-stream">
        <div className="console-panel-head"><div><span className="status-dot" /><h2>Live Visitor Stream</h2></div><small>{data.total_visits} tracked visits · refreshes every 10 seconds</small></div>
